...
for k in range(num_trials):
    ...
    agent = DQNAgent()
    trained = 800
    with open('qn_statedist_nstep.pkl','rb') as f:
        episode_rewards = list(pickle.load(f))
        assert len(episode_rewards) == trained

    if agent.isTraining:
        scores_window = deque(maxlen=100)
        start = time.time()
        for episode in range(trained+1, episodes + 1):
            rewards = 0
            state = env.reset()
            ...
            if episode % 50 == 0:
                ...
                with open('qn_statedist_nstep.pkl','wb') as f:
                    pickle.dump(np.array(episode_rewards), f)
            if episode % 100 == 0:
...
plt.xlabel('Episodes')
plt.ylabel('Rewards')
plt.savefig('QN_statedist_nstep.png')
print('plotted')
